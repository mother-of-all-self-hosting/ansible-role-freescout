...
#
# Tags look like `v<FreeScout version>-<release>`:
#
# - if defaults/main.yml points at a FreeScout version that has never been
#   released, the release counter restarts at 0 (`v1.8.236-0`)
# - otherwise the counter is incremented (`v1.8.236-1`), but only if something
...
#
# `freescout_version` is the variable to read, because it is the leaf literal
# that carries the `# renovate:` annotation and is what Renovate rewrites.
# Everything else (`freescout_container_image_tag`, `freescout_container_image`)
# is Jinja derived from it and would not resolve outside of Ansible.
...
version="$(sed -nE 's|^freescout_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the FreeScout version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v1.8.235`),
